...
uint32_t
pk_init_biquad_filter(biquad_filt_f32_t *biquadCtx)
{
    /**
...
uint32_t
pk_apply_biquad_filter(biquad_filt_f32_t *biquadCtx, float32_t *pSrc, float32_t *pResult, uint32_t blockSize)
{
    /**
...
uint32_t
pk_apply_biquad_filtfilt(biquad_filt_f32_t *biquadCtx, float32_t *pSrc, float32_t *pResult, uint32_t blockSize, float32_t *state)
{
    /**
...
pk_ecg_find_peaks(ecg_peak_f32_t *ctx, float32_t *ecg, uint32_t ecgLen, uint32_t *peaks)
{
    ...
    // ns_printf("qrs_grad = np.array([\n");
    // for (size_t i = 0; i < ecgLen; i++){
    //     ns_printf("%.1f,", qrsGrad[i]);
    //     if (i % 40 == 39){ ns_printf("\n"); }
    // }
    // ns_printf("])\n\n");

    // ns_printf("avg_grad = np.array([\n");
    // for (size_t i = 0; i < ecgLen; i++){
    //     ns_printf("%.1f,", avgGrad[i]);
    //     if (i % 40 == 39){ ns_printf("\n"); }
    // }
    // ns_printf("])\n\n");
...
float32_t
pk_compute_spo2_in_time(float32_t *ppg1, float32_t *ppg2, float32_t ppg1Mean, float32_t ppg2Mean, uint32_t blockSize, float32_t *coefs, float32_t sampleRate)
{
